%This function implements the Levenberg-Marquardt method
%Inputs:
...
%   increase_step->     The increase step for the algorithm
...
function [x_values, f_values, grad_values]=LM(f, gradf, hessf, x0, e, increase_step, gamma_method, gamma_params)

    %Working with the arguments
    x0=x0(:);
    fprintf("Implementing Levenberg Marquardt on f, with x0=(%d,%d) and e=%d\n", x0(1), x0(2), e);
    ...
    while(n<1000)
        ...
        current_hessian=force_pos_def(current_hessian, increase_step);      %Forcing the matrix to be positive definite
