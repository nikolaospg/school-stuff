...
%Using function handles to initialise the derivatives of the objective functions
der_f1=@(x)( 2.*(x-3) +2.*sin(x+3).*cos(x+3));
der_f2=@(x)( cos(x/2) - sin(x/2).*(x-1)/2 +2*x);
der_f3=@(x)( 2*(x+2) + exp(x-2).*(sin(x+3) + cos(x+3)));
...
[a_vector1, b_vector1, k1, f_comp1] = bisection_der(der_f1, a, b, l);
[a_vector2, b_vector2, k2, f_comp2] = bisection_der(der_f2, a, b, l);
[a_vector3, b_vector3, k3, f_comp3] = bisection_der(der_f3, a, b, l);
...
%I choose the same l values I chose on the first exercise (comparing on the same base(
initial_l=0.01;
step_l=0.005;
final_l=(b-a)/2;
...
for i=1:length(varying_l)
    [~,~,~,f1_comps_l(i)]=bisection_der(der_f1,a,b,varying_l(i));
    [~,~,~,f2_comps_l(i)]=bisection_der(der_f2,a,b,varying_l(i));
    [~,~,~,f3_comps_l(i)]=bisection_der(der_f3,a,b,varying_l(i));
end
...
for i=1:length(varying_l)
    [a1_vector_n, b1_vector_n, n1 ,~]=bisection_der(der_f1,a,b,varying_l(i));
    [a2_vector_n, b2_vector_n, n2 ,~]=bisection_der(der_f2,a,b,varying_l(i));
    [a3_vector_n, b3_vector_n, n3 ,~]=bisection_der(der_f3,a,b,varying_l(i));
